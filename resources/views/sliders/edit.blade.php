    ...
    <div class="content-wrapper">
        ...
        <input type="hidden" name="edit_id" id="edit-id" value="{{ $customField->id }}" />
                            <div class="row">
                                ...
                                <div class="form-group col-sm-12 col-md-6">
                                    <label>{{ __('type') }} <span class="text-danger"> {{ __('*') }} </span></label>
                                    <select name="type" id="edit-type-select" class="form-control edit-type-field"
                                        ...
                                        <option value="number" {{ $customField->type === 'number' ? 'selected' : '' }}>
                                            {{ __('Numeric') }}</option>
                                        <option value="textbox" {{ $customField->type === 'textbox' ? 'selected' : '' }}>
                                            {{ __('Text') }}</option>
                                        <option value="fileinput"
                                            {{ $customField->type === 'fileinput' ? 'selected' : '' }}>
                                    ...
                                    <div class="row edit-option-section" data-repeater-item>
                                        ...
                                        <div class="form-group col-md-1 pl-0 mt-4">
                                            <button data-repeater-delete type="button"
                                                ...
                                                {{ count($defaultValues) <= 1 ? 'disabled' : '' }}>
    ...
    <script>
        ...
        $(document).ready(function() {
            window.validateInput = function(event) {
                ...
                if (!(charCode > {{ __('= 65 && charCode') }} <= 90) && !(charCode > {{ __('= 97 && charCode') }} <= 122) && !(
                        charCode === 32)) {
        ...
        editDefaultValuesRepeater.setList([
            @foreach ($defaultValues as $data)
                    ...
                    }@if (!$loop-> {{ __('last),@endif
                @endforeach 
        ])
        // $(\'.file_type\').trigger(\'change\');') }} </script>
@endsection
