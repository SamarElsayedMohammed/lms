...
@section('content')
    <section class="section">
        <div class="section-header">
            <h1>Add Chapter to: {{ $course->name }}</h1>
        ...
        <div class="section-body">
            <h2 class="section-title"> {{ __('Create New Chapter') }} </h2>
            <p class="section-lead"> {{ __('Add a new chapter to organize your course content.') }} </p>
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form action="{{ route('courses.chapters.store', $course->id) }}" method="POST"> @csrf <div class="form-group row mb-4">
                                    ...
                                    <div class="col-sm-12 col-md-7">
                                        <input type="text" name="title"
                                            ...
                                            value="{{ old('title') }}" required> @error(\'title\') <div class="invalid-feedback">{{ $message }}</div> @enderror </div>
                                </div>

                                <div class="form-group row mb-4">
                                    <label class="col-form-label text-md-right col-12 col-md-3 col-lg-3"> {{ __('Description') }} </label>
                                    <div class="col-sm-12 col-md-7">
                                        <textarea name="description" class="form-control @error('description') is-invalid @enderror" style="height: 120px;">{{ old('description') }}</textarea> @error(\'description\') <div class="invalid-feedback">{{ $message }}</div> @enderror </div>
                                </div>
                                ...
                                <div class="form-group row mb-4">
                                    ...
                                    <div class="col-sm-12 col-md-7">
                                        <input type="number" name="order"
                                            ...
                                            value="{{ old('order', $nextOrder) }}" min="0">
                                        <small class="form-text text-muted"> {{ __('The order in which this chapter will appear. Leave as is to add to the end.') }} </small> @error(\'order\') <div class="invalid-feedback">{{ $message }}</div> @enderror </div>
                                </div>
                                    ...
                                    <div class="col-sm-12 col-md-7">
                                        <label class="custom-switch mt-2">
                                            <input type="checkbox" name="free_preview" value="1"
                                                class="custom-switch-input" {{ old('free_preview') ? 'checked' : '' }}>
                                ...
                                <div class="form-group row mb-4">
                                    ...
                                    <div class="col-sm-12 col-md-7">
                                        <button type="submit" class="btn btn-primary"> {{ __('Create Chapter') }} </button>
