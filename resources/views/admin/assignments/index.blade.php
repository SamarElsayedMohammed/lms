...
<section class="section">
    <div class="section-body">
                    ...
                    <div class="row g-3 align-items-end">
                        ...
                        <div class="col-md-3 col-sm-6">
                            ...
                            <select name="instructor_id" class="form-control">
                                ...
                                <option value="{{ $instructor->id }}" {{ request('instructor_id')==$instructor->id ?
                                    'selected' : '' }}>
                                    {{ $instructor->name }}
                                </option>
                                @endforeach
        ...
        <div class="card">
            <div class="card-header">
                <h4>{{ __('Assignment Submissions') }} ({{ $submissions->total() }})</h4>
                <div class="card-header-action">
                    <button class="btn btn-success" onclick="bulkAccept()">{{ __('Bulk Accept') }}</button>
                    <button class="btn btn-danger" onclick="bulkReject()">{{ __('Bulk Reject') }}</button>
                    <button class="btn btn-warning" onclick="bulkSuspend()">{{ __('Bulk Suspend') }}</button>
                </div>
            </div>
            <div class="card-body">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        <div class="avatar avatar-sm mr-3">
                                            <img src="{{ $submission->user->profile ? asset('storage/' . $submission->user->profile) : asset('img/avatar/avatar-1.png') }}"
                                                alt="avatar">
                                        </div>
                                        <div>
                                ...
                                <td>
                                    ...
                                    <div class="text-small text-muted">{{ __('Max Points:') }} {{
                                        $submission->assignment->points ?? 'N/A' }}</div>
                                </td>
                                <td>
                                    <div class="font-weight-600">
                                        @if($submission->assignment && $submission->assignment->chapter &&
                                        $submission->assignment->chapter->course)
                                        {{ $submission->assignment->chapter->course->title }}
                                        @else
                                        <span class="text-muted">{{ __('Course not found') }}</span>
                                        @endif
                                    </div>
                                </td>
                                <td>
                                    @if($submission->assignment && $submission->assignment->chapter &&
                                    $submission->assignment->chapter->course &&
                                    $submission->assignment->chapter->course->user)
                                    <div class="font-weight-600">{{ $submission->assignment->chapter->course->user->name
                                        }}</div>
                                    <div class="text-small text-muted">{{
                                        $submission->assignment->chapter->course->user->email }}</div>
                                    @else
                                    <span class="text-muted">{{ __('Instructor not found') }}</span>
                                    @endif
                                </td>
                                <td>
                                    ...
                                    @else
                                    <span class="badge badge-secondary">{{ ucfirst($submission->status ?? __('Unknown'))
                                        }}</span>
                                    @endif
                                </td>
                                <td>
                                ...
                                <td>
                                    <a href="{{ route('admin.assignments.show', $submission->id) }}"
                ...
                <div class="d-flex justify-content-center mt-4">
                    <nav aria-label="Page navigation">
                        <ul class="pagination pagination-md mb-0">
                            ...
                            <li class="page-item">
                                <a class="page-link"
                                    href="{{ $submissions->appends(request()->query())->previousPageUrl() }}"
                            ...
                            if ($endPage < $lastPage) { $showLastPage=true; } else { $showLastPage=false; } @endphp {{--
                                First Page --}} @if($showFirstPage) <li class="page-item">
                                <a class="page-link"
                                    href="{{ $submissions->appends(request()->query())->url(1) }}">1</a>
                                </li>
                                @if($startPage > 2)
                                <li class="page-item disabled">
                                ...
                                @for ($page = $startPage; $page <= $endPage; $page++) @if ($page==$currentPage) <li
                                    ...
                                    <li class="page-item">
                                        <a class="page-link"
                                            href="{{ $submissions->appends(request()->query())->url($page) }}">{{ $page
                                            }}</a>
                                    </li>
                                    @endif
                                    @endfor
                                    ...
                                    {{-- Last Page --}}
                                    @if($showLastPage)
                                    @if($endPage < $lastPage - 1) <li class="page-item disabled">
                                        <span class="page-link">...</span>
                                        </li>
                                        @endif
                                        <li class="page-item">
                                            <a class="page-link"
                                                href="{{ $submissions->appends(request()->query())->url($lastPage) }}">{{
                                                $lastPage }}</a>
                                        </li>
                                        @endif
                                        ...
                                        <li class="page-item">
                                            <a class="page-link"
                                                href="{{ $submissions->appends(request()->query())->nextPageUrl() }}"
                                                aria-label="{{ __('Next') }}">
                ...
                <div class="d-flex justify-content-center mt-2">
                    <p class="text-muted small">
                        {{ __('Showing :first to :last of :total results', ['first' => $submissions->firstItem(), 'last'
                        => $submissions->lastItem(), 'total' => $submissions->total()]) }}
                    </p>
                </div>
                @endif
                @else
                <div class="text-center py-4">
                    <h5>{{ __('No assignment submissions found') }}</h5>
                    <p class="text-muted">
                        @if($submissions->total() == 0)
                        {{ __('There are no assignment submissions in the system yet.') }}
                        <br><br>
                        <strong>{{ __('To get started:') }}</strong><br>
                        {{ __('1. Create a course with assignments') }}<br>
                        {{ __('2. Have students submit assignments') }}<br>
                        {{ __('3. Submissions will appear here for review') }}
                        @else
                        {{ __('There are no assignment submissions matching your current filters.') }}
                        <br><br>
                        <a href="{{ route('admin.assignments.index') }}" class="btn btn-primary">{{ __('Clear Filters')
                            }}</a>
                        @endif
                    </p>
                </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="bulkActionTitle">{{ __('Bulk Action') }}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="{{ __('Close') }}"
                    style="display: block !important; visibility: visible !important; opacity: 1 !important;">
                    <span aria-hidden="true"
                        style="font-size: 1.5rem; font-weight: 700; line-height: 1; color: #000; text-shadow: 0 1px 0 #fff;">&times;</span>
                </button>
            </div>
            <form id="bulkActionForm" method="POST" action="{{ route('admin.assignments.bulk-update') }}">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label>{{ __('Rejection Reason (for rejected/suspended submissions)') }} <span
                                class="text-danger" id="bulk-feedback-required" style="display: none;">*</span></label>
                        <textarea name="feedback" id="bulkFeedback" class="form-control" rows="3"></textarea>
                        <small class="form-text text-muted">{{ __('Required when status is Reject or Suspend')
                            }}</small>
                    </div>
...
<script>
    ...
    function bulkAccept() {
        ...
        document.getElementById('bulkActionTitle').textContent = '{{ __("Bulk Accept Submissions") }}';
    ...
    function bulkReject() {
        ...
        document.getElementById('bulkActionTitle').textContent = '{{ __("Bulk Reject Submissions") }}';
        document.getElementById('bulkActionForm').action = '{{ route("admin.assignments.bulk-update") }}';
    ...
    function bulkSuspend() {
        ...
        document.getElementById('bulkActionTitle').textContent = '{{ __("Bulk Suspend Submissions") }}';
        document.getElementById('bulkActionForm').action = '{{ route("admin.assignments.bulk-update") }}';
