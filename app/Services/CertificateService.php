...
class CertificateService
{
     ...
     * Rule: 100% video lectures completed.
     */
     ...
     * Delegates to the canonical issueCertificate pipeline.
     */
     ...
     * - Authorized Admin issuance
     * - System backfill / repair
     *
     * Invariants:
     * - 18-digit random numeric certificate_number (e.g. 583104927641805273)
     * - 32-character hex verification_token
     * - 10-character verification_code
     * - Immutable snapshot: student_name, arabic_title, english_title, instructor_name, dates
     * - Distributed lock + DB transaction boundary for concurrency safety and idempotency
     */
    public function issueCertificate(int $userId, int $courseId, array $options = []): ?CourseCertificate
    {
        try {
            $user   = User::findOrFail($userId);
            $course = Course::with('user')->findOrFail($courseId);
            $issuanceSource = $options['issuance_source'] ?? 'automatic';
            $issuerId       = $options['issuer_id'] ?? ($issuanceSource === 'admin_manual' ? \Illuminate\Support\Facades\Auth::id() : ($course->user_id ?? null));
            ...
            if ($issuanceSource === 'automatic') {
                ...
                if (!$allowIncomplete && !$this->checkCourseCompletionStatus($userId, $courseId)) {
                    $progress = app(CourseProgressService::class)->getProgressWithCache($userId, $courseId);
                    if ($progress->progress_percentage < 100 && $progress->status !== 'completed') {
                        Log::info("Admin manual issuance note: user {$userId} course {$courseId} progress {$progress->progress_percentage}%");
                    }
                }
            }
            ...
            return $lock->block(10, function () use ($userId, $courseId, $user, $course, $issuanceSource, $issuerId, $options) {
                ...
                // 18-digit random numeric certificate number
                ...
                $appUrl = rtrim(config('app.frontend_url') ?: config('app.url'), '/');
                $verificationUrl = $appUrl . '/verify-certificate?code=' . $certificateNumber;
                ...
                $studentName = trim($options['student_name'] ?? ($user->name ?? 'Student'));
                $arabicTitle = trim($options['arabic_title'] ?? ($course->title ?? 'Course'));
                $englishTitle = trim($options['english_title'] ?? ($course->title ?? 'Course'));
                ...
                try {
                    ...
                    $result   = (new \Endroid\QrCode\Builder\Builder(data: $qrTarget, size: 150))->build();
                ...
                if ($issuanceSource === 'automatic' || ($options['notify_user'] ?? true)) {
            ...
            // Check if record was already created concurrently by another process
            $existing = CourseCertificate::where('user_id', $userId)
                ->where('course_id', $courseId)
                ->first();
    ...
    public static function generateCertificateHtml($template, $certificate): string
    {
        ...
        if (empty($bgDataBase64)) {
            ...
            if (file_exists($defaultBgPath)) {
                ...
                $bgDataBase64 = self::DEFAULT_BG_BASE64;
            }
        }
